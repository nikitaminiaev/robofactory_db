...
class WebSocketServer:
    ...
    def _handle_client(self, client_socket, addr):
        ...
        if request.startswith(b'GET '):
            ...
            try:
                ...
                log(f"Получено простое сообщение от {addr}: {message}")
                ...
                try:
                    ...
                    log(f"Получен JSON-объект: {json_data}")
                    ...
                    if isinstance(json_data, dict) and json_data.get("type") == "freecad_command":
                        log(f"Получена команда для FreeCad: {json_data.get('python_code', '')}")
                ...
                log(f"Ошибка при обработке простого сообщения: {e}")
            finally:
                client_socket.close()
    ...
    def _handle_websocket_connection(self, client_socket, addr):
        ...
        try:
            ...
            log(f"WebSocket-клиент {addr} добавлен. Всего клиентов: {len(self.connected_clients)}")
            ...
            def send_message(message):
               ...
               try:
                   ...
                   log(f"Ошибка при логировании исходящего сообщения: {e}")
            ...
            def receive_message():
               ...
               try:
                   ...
                   log("Ошибка декодирования UTF-8")
                   return None    
            ...
            while True:
                ...
                log(f"Получено от WebSocket-клиента {addr}: {message}")
            ...
            log(f"Ошибка при обработке WebSocket-сообщений: {e}")
        finally:
            ...
            if client_socket in self.connected_clients:
                ...
                log(f"WebSocket-клиент {addr} удален. Осталось клиентов: {len(self.connected_clients)}")
            client_socket.close()
    ...
    def _broadcast_message(self, message):
        ...
        if not self.connected_clients:
            log("Нет подключенных WebSocket-клиентов для отправки сообщения")
            return
        
        log(f"Отправка сообщения '{message}' всем {len(self.connected_clients)} клиентам")
        ...
        try:
            ...
            log(f"Ошибка при логировании широковещательного сообщения: {e}")
        ...
        for client in self.connected_clients:
            try:
                ...
                log(f"Сообщение успешно отправлено клиенту")
            except Exception as e:
                log(f"Ошибка при отправке сообщения клиенту: {e}")
        ...
        if disconnected_clients:
            log(f"Удалено {len(disconnected_clients)} отключенных клиентов. Осталось: {len(self.connected_clients)}")
    ...
    def start(self):
        ...
        if self.server:
            log(f"Сервер уже запущен на {self.host}:{self.port}")
            return
        ...
        try:
            ...
            try:
                ...
                log(f"Ошибка при привязке к порту {self.port}: {e}")
                # Порт может быть уже занят, проверяем
                if self.is_running():
                    log(f"Сервер уже запущен на {self.host}:{self.port}, используем существующий")
                    self.server = None
                    return
                raise  # Если порт занят, но сервер не запущен - поднимаем ошибку
                
            self.server.listen(5)
            log(f"Сервер запущен на {self.host}:{self.port}")
            ...
            while True:
                ...
                try:
                    ...
                    log(f"Ошибка при принятии соединения: {e}")
            ...
            log("Сервер остановлен по запросу пользователя")
        except Exception as e:
            log(f"Ошибка при запуске сервера: {e}")
        finally:
            self.stop()
        ...
        with self._lock:  # Используем блокировку для безопасного завершения
            if self.server:
                try:
                    ...
                    self.server.close()
                    log("Серверный сокет закрыт")
                except Exception as e:
                    log(f"Ошибка при закрытии серверного сокета: {e}")
                finally:
                    self.server = None
            ...
            for client in list(self.connected_clients):  # Используем копию списка
                try:
                    ...
                    log(f"Ошибка при закрытии клиентского соединения: {e}")
            
            if disconnected_count > 0:
                log(f"Закрыто {disconnected_count} клиентских соединений")
            ...
            log("Сервер полностью остановлен")
